...
class NewspaperID(Resource):
    ...
    @newspaper_ns.doc(paper_get_model, description="Get a new newspaper")
    @newspaper_ns.marshal_with(paper_get_model, envelope='newspaper')
...
class IssueAPI(Resource):
    ...
    @newspaper_ns.doc(issue_get_model, description='List all issues of a specific newspaper')
    @newspaper_ns.marshal_list_with(issue_get_model)
    def get(self, paper_id):
        targeted_paper = Agency.get_instance().get_newspaper(paper_id)
        if targeted_paper == 404:
            abort(404, f'Newspaper with ID {paper_id} was NOT found')
...
class IssueID(Resource):
    @newspaper_ns.doc(issue_get_model, description='Get info of a newspaper issue')
